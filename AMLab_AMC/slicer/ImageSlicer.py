...
class ImageSlicer(object):
    ...
    def __init__(self, source, size, strides=[None, None], BATCH = False, PADDING=False,labels=False):
        self.source = source
        self.size = size
    ...
    def save_images(self,transformed, save_dir):
        if not(os.path.exists(save_dir)):
            ...
            for key, val in transformed.items():
                ...
                #print(type(key))
                print("the value yo: ", os.path.isdir(path))
                print("val: ", val)
                if os.path.isdir(path):
                    key=str(int(key)+1)
                    path = os.path.join(save_dir, key)
                    print("new path bitches: ", path)
                    os.mkdir(path)
                else:
                    print("new file: ",path)
                    os.mkdir(path)
                for i, j in enumerate(val):
                    if self.labels:
                        #imsave(os.path.join(path, str(i+1)+'.png'),j)
                        plt.imsave(os.path.join(path, str(i+1)+'.png'), j,cmap=cm.gray)
                    else:
                        #imsave(os.path.join(path, str(i+1)+'.png'),j)
                        #print(j.shape)
                        #raise ValueError("these times")
                        plt.imsave(os.path.join(path, str(i+1)+'.png'), j)
    
    # saves subimages from multiple sources under one directory
    # in an incremental fashion
    def save_images_directory(self,transformed, save_dir):
        i = len(os.listdir(save_dir)) + 1
        print
        if not(os.path.exists(save_dir)):
            raise Exception("Path does not exist!")
        else:
            for _, val in transformed.items():
                for j in val:
                    if self.labels:
                        plt.imsave(os.path.join(save_dir, str(i)+'.png'), j,cmap=cm.gray)
                    else:
                        plt.imsave(os.path.join(save_dir, str(i)+'.png'), j)
                    i += 1
